...
- Tests: `ctest --test-dir build --output-on-failure` (~0.05s, 29 tests)
- ASan: `cmake -S . -B build-asan -DCMAKE_BUILD_TYPE=Debug -DCMAKE_C_FLAGS="-fsanitize=address -fno-omit-frame-pointer -g" -DCMAKE_EXE_LINKER_FLAGS="-fsanitize=address"`
  - Run with `LD_PRELOAD=$(gcc -print-file-name=libasan.so) ASAN_OPTIONS=detect_leaks=0 ./build-asan/bin/test_X` (LSan not supported in agent sandbox; CI runs full ASan).
...
- linked_queue now has a bounded per-queue freelist (cap 4096) — enqueue/dequeue reuse nodes.
...
- DONE 2026-05-01: linked_queue node freelist (PR pending; this run; ~2.8–3.3× mixed enq/deq).
...
- TODO: matrix.c (1005 LOC) — SIMD opportunities (test_vector_simd hints work started).
...
- TODO: deque resize uses per-element `%`; could use two memcpys when wrapped.
...
- 2026-05 monthly activity issue created this run.
...
- 2026-05-01 20:50 UTC: linked_queue freelist PR (mixed enq/deq 15.0→5.3 ms, reuse 17.6→5.3 ms; burst capped at 4096 nodes is intentional).
