...
void test_graph_add_edge_implicit_vertices(graph_representation rep) {
    ...
    print_test_result(g->vertex_count(g) == 2, "graph_add_edge implicitly adds missing vertices");
    print_test_result(g->has_edge(g, 5, 6), "directed edge exists after add");
    print_test_result(!g->has_edge(g, 6, 5), "reverse edge should not exist in directed graph");
...
void test_graph_basic_undirected(graph_representation rep) {
    ...
    g->add_edge(g, 0, 1, 1.5);
    print_test_result(g->has_edge(g, 0, 1) && g->has_edge(g, 1, 0), "undirected add_edge creates symmetric edges");
    print_test_result(g->get_edge_weight(g, 0, 1) == 1.5, "edge weight is stored (0->1)");

    g->remove_edge(g, 0, 1);
    print_test_result(!g->has_edge(g, 0, 1) && !g->has_edge(g, 1, 0), "graph_remove_edge removes symmetric edges");

    g->add_edge(g, 0, 1, 1.0);
    g->add_edge(g, 0, 2, 1.0);
    g->add_edge(g, 0, 3, 1.0); /* also tests implicit vertex addition */

    list *neighbors = g->get_neighbors(g, 0);
