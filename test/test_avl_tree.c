...
static void test_avl_tree_basic_ops() {
    ...
    print_test_result(found && *found == 500, "AVL tree search returns correct value");

    int update_key = 15;
    int update_value = 999;
    tree_insert(t, &update_key, &update_value);
    int *updated = (int *)tree_search(t, &update_key);
    print_test_result(updated && *updated == 999, "AVL tree insert updates existing key value");

    int *minv = (int *)tree_min(t);
    int *maxv = (int *)tree_max(t);
    print_test_result(minv && *minv == 30, "AVL tree min returns smallest key value");
    print_test_result(maxv && *maxv == 1000, "AVL tree max returns largest key value");

    int pred_key = 15;
    int succ_key = 15;
    int *pred = (int *)tree_predecessor(t, &pred_key);
    int *succ = (int *)tree_successor(t, &succ_key);
    print_test_result(pred && *pred == 100, "AVL tree predecessor returns correct value");
    print_test_result(succ && *succ == 200, "AVL tree successor returns correct value");

    int missing_key = 16;
    int *pred2 = (int *)tree_predecessor(t, &missing_key);
    int *succ2 = (int *)tree_successor(t, &missing_key);
    print_test_result(pred2 && *pred2 == 999, "AVL predecessor works for missing key");
    print_test_result(succ2 && *succ2 == 200, "AVL successor works for missing key");
    ...
    print_test_result(range_ok, "AVL tree range query returns ordered values");
    if (range) {
        range->free(range);
    }

    int expected_keys1[] = {3, 4, 10, 15, 20, 50, 70, 100};
    print_test_result(assert_in_order_keys(t, expected_keys1, 8), "AVL in-order traversal yields sorted keys");

    int delete_key = 3;
    tree_delete(t, &delete_key);
    print_test_result(tree_search(t, &delete_key) == NULL, "AVL delete removes leaf node");

    int delete_key2 = 70;
    tree_delete(t, &delete_key2);
    print_test_result(tree_search(t, &delete_key2) == NULL, "AVL delete removes internal node");

    int delete_key3 = 20;
    tree_delete(t, &delete_key3);
    print_test_result(tree_search(t, &delete_key3) == NULL, "AVL delete removes two-child node");

    int expected_keys2[] = {4, 10, 15, 50, 100};
    print_test_result(assert_in_order_keys(t, expected_keys2, 5), "AVL traversal remains sorted after deletes");
    print_test_result(tree_size(t) == 5, "AVL tree_size matches remaining items");

    int missing_delete = 123;
    tree_delete(t, &missing_delete);
    print_test_result(tree_size(t) == 5, "AVL delete on missing key is a no-op");
